...
const Card = ({ product }) => {
  return (
    <div className="bg-background border border-border rounded-2xl shadow-md overflow-hidden">
      <a href="#">
        <img className="w-full h-40 object-cover p-4 rounded-t-xl" src={product.img} alt={product.name} />
      </a>
      <div className="px-6 pb-6">
        ...
        <div className="flex items-center justify-between mt-4">
          <span className="text-2xl font-bold text-textMain">{product.price}</span>
        </div>
      </div>
